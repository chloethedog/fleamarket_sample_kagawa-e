class User < ApplicationRecord
  ...
  validates :email, presence: true, uniqueness: true, format: { with: /\A\S+@\S+\.\S+\z/, message: 'は＠とドメインを含む必要があります'}
                    
  validates :encrypted_password, confirmation: true
  validates :encrypted_password, :encrypted_password_confirmation ,presence: true ,length: { minimum: 7 }
